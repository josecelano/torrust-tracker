...
| T8  | TODO   | Add shared event bus integration test                       | Start two HTTP/UDP instances with `tracker_usage_statistics=true`, set up a single event listener on the shared broadcaster, announce to both, verify the listener receives events from both instances. Validates the shared broadcaster pattern from [ADR 20260727180000](../adrs/20260727180000_shared_services_across_tracker_instances.md). See also `tests/servers/api/contract/stats/mod.rs` TODO. |
